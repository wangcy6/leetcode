...
public:
  ...
  int dfs(vector<vector<int>> &matrix, int row, int col, int pre, int rows,
          int cols) {
    ...
    matrix[row][col] =
        pre; //错误3 破坏当前节点数据，后面其他节点无法在使用该节点
    return isBoth;
  }
  int dfs(vector<vector<int>> &matrix, int row, int col, int pre) {
    ...
    matrix[row][col] = INT_MAX;
    int isBoth =
        dfs(matrix, row, col - 1, pre) | dfs(matrix, row, col + 1, pre) |
        dfs(matrix, row + 1, col, pre) | dfs(matrix, row - 1, col, pre);
